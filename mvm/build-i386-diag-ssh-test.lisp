...
(let* ((ssh-main (format nil "~{~A~%~}"
                        (list
                         ...
                         "  (enable-pit-timer)"
                         "  (net-actor-main))"
                         ;; Override net-actor-main to not call yield
...
")
       ;; Combine all source: networking + REPL + console + e1000 raw + SSH main
       ;; Order matters: i386-e1000-raw MUST come after e1000.lisp AND i386-console
  ...
  (format t "Functions callable from REPL: ~D~%"
          (length (remove-if (lambda (d)
                               (or (string= (car d) "EVAL-PLATFORM-CALL")
                                   (> (cdr d) 5)))
                             defuns)))
  (let ((image (build-image :target :i386 :source-text final-source)))
