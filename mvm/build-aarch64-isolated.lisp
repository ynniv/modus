...
;;;;
;;;; Produces /tmp/modus-aarch64-isolated.bin — boot with:
;;;;   qemu-system-aarch64 -machine virt -cpu cortex-a57 -m 512 \
;;;;     -kernel /tmp/modus-aarch64-isolated.bin -nographic -semihosting \
...
(let* ((ssh-main (format nil "~{~A~%~}"
  ...
  (let ((image (build-image :target :aarch64 :source-text combined-source)))
    ...
    (write-kernel-image image "/tmp/modus-aarch64-isolated.bin")
    (format t "Done. Boot with:~%")
    (format t "  qemu-system-aarch64 -machine virt -cpu cortex-a57 -m 512 \\~%")
    (format t "    -kernel /tmp/modus-aarch64-isolated.bin -nographic -semihosting \\~%")
