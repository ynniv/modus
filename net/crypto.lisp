...
(defun sha256-block (block bo h)
  (let ((w (make-array 256)))
    ...
    ;; Pre-compute aref calls into let bindings to avoid nested calls inside logior
    (dotimes (i 16)
      (let ((j (+ bo (* i 4))))
        (let ((b0 (aref block j)))
          (let ((b1 (aref block (+ j 1))))
            (let ((b2 (aref block (+ j 2))))
              (let ((b3 (aref block (+ j 3))))
                (buf-write-u32 w (* i 4)
                  (logior (ash b0 24)
                          (logior (ash b1 16)
                                  (logior (ash b2 8) b3))))))))))
    ;; Expand W[16..63]
    ;; Pre-compute function calls into let bindings to avoid nested calls inside +
    (let ((i 16))
      (loop
        (when (not (< i 64)) (return ()))
        (let ((s0 (sha256-lsig0 (buf-read-u32 w (* (- i 15) 4)))))
          (let ((s1 (sha256-lsig1 (buf-read-u32 w (* (- i 2) 4)))))
            (let ((w7 (buf-read-u32 w (* (- i 7) 4))))
              (let ((w16 (buf-read-u32 w (* (- i 16) 4))))
                (buf-write-u32 w (* i 4)
                  (logand (+ s1 (+ w7 (+ s0 w16)))
                          #xFFFFFFFF))))))
        (setq i (+ i 1))))
    ...
    (let ((a (buf-read-u32 h 0))
      ...
      ;; 64 rounds
      ;; Pre-compute function calls into let bindings to avoid nested calls inside +
      ;; (MVM compile-add + compile-call interaction bug on AArch64 bare metal)
      (dotimes (i 64)
        (let ((bsig1 (sha256-bsig1 e)))
          (let ((ch-val (sha256-ch e f g)))
            (let ((ki (mem-ref (+ (e1000-state-base) (+ #x100 (* i 4))) :u32)))
              (let ((wi (buf-read-u32 w (* i 4))))
                (let ((t1 (logand (+ hh (+ bsig1 (+ ch-val (+ ki wi)))) #xFFFFFFFF)))
                  (let ((bsig0 (sha256-bsig0 a)))
                    (let ((maj-val (sha256-maj a b cc)))
                      (let ((t2 (logand (+ bsig0 maj-val) #xFFFFFFFF)))
                        (setq hh g)
                        (setq g f)
                        (setq f e)
      ...
      (let ((ks (chacha-block key nonce (+ counter block-num)))
        ...
        (dotimes (i 64)
          (when (< (+ base i) data-len)
            (let ((di (aref data (+ base i)))
                  (ki (aref ks i)))
              (aset result (+ base i) (logxor di ki))))))
...
(defun poly-add-limbs (ha hb)
  (dotimes (i 5)
    (let ((off (* i 4)))
      (let ((va (buf-read-u32 ha off))
            (vb (buf-read-u32 hb off)))
        (buf-write-u32 ha off (+ va vb))))))
...
(defun sha512-block (block bo h w)
      ...
      (let ((b0 (aref block j)))
        (let ((b1 (aref block (+ j 1))))
          (let ((b2 (aref block (+ j 2))))
            (let ((b3 (aref block (+ j 3))))
              (buf-write-u32 w (* i 8)
                (logior (ash b0 24) (logior (ash b1 16) (logior (ash b2 8) b3))))))))
      (let ((b4 (aref block (+ j 4))))
        (let ((b5 (aref block (+ j 5))))
          (let ((b6 (aref block (+ j 6))))
            (let ((b7 (aref block (+ j 7))))
              (buf-write-u32 w (+ (* i 8) 4)
                (logior (ash b4 24) (logior (ash b5 16) (logior (ash b6 8) b7))))))))))
    ...
    (loop
      ...
      (let ((im2 (* (- i 2) 8)))
        (let ((im7 (* (- i 7) 8)))
          (let ((im15 (* (- i 15) 8)))
            (let ((im16 (* (- i 16) 8)))
              (let ((s1 (sha512-lsig1 (cons (buf-read-u32 w im2) (buf-read-u32 w (+ im2 4))))))
                (let ((s0 (sha512-lsig0 (cons (buf-read-u32 w im15) (buf-read-u32 w (+ im15 4))))))
                  (let ((w7 (cons (buf-read-u32 w im7) (buf-read-u32 w (+ im7 4)))))
                    (let ((w16 (cons (buf-read-u32 w im16) (buf-read-u32 w (+ im16 4)))))
                      (let ((sum1 (u64-add s1 w7)))
                        (let ((sum2 (u64-add s0 w16)))
                          (let ((result (u64-add sum1 sum2)))
  ...
  (let ((a (cons (buf-read-u32 h 0) (buf-read-u32 h 4)))
    ...
    ;; 80 rounds
    ;; Pre-compute function calls into let bindings to avoid nested calls
    ;; (MVM compile-add + compile-call interaction bug on AArch64 bare metal)
    (dotimes (i 80)
      (let ((kaddr (+ (e1000-state-base) (+ #x200 (* i 8)))))
        (let ((ki (cons (mem-ref kaddr :u32) (mem-ref (+ kaddr 4) :u32))))
          (let ((wioff (* i 8)))
            (let ((wi (cons (buf-read-u32 w wioff) (buf-read-u32 w (+ wioff 4)))))
              (let ((sig1 (sha512-sigma1 e)))
                (let ((ch-val (sha512-ch e f g)))
                  (let ((kw (u64-add ki wi)))
                    (let ((ch-kw (u64-add ch-val kw)))
                      (let ((h-sig1 (u64-add hh sig1)))
                        (let ((t1 (u64-add h-sig1 ch-kw)))
                          (let ((sig0 (sha512-sigma0 a)))
                            (let ((maj-val (sha512-maj a b cc)))
                              (let ((t2 (u64-add sig0 maj-val)))
                                (setq hh g)
                                (setq g f)
                                (setq f e)
                                (setq e (u64-add d t1))
...
(defun ed25519-init ()
  ...
  (when (zerop (mem-ref (+ (e1000-state-base) #x5D0) :u32))
    ...
    ;; Base point X (SBCL-verified: 26/25/26/25... bit limbs of standard Bx)
    ...
    ;; Base point Y (SBCL-verified: 26/25/26/25... bit limbs of standard By)
        ...
        (dotimes (j 32)
          (let ((idx (+ i j)))
            (when (< idx 64)
              (let ((ai (aref a i))
                    (bj (aref b j))
                    (pidx (aref product idx)))
                (let ((pp (+ (* ai bj) (+ pidx carry))))
