...
(defvar *command-counter* 0
  "Genera-style command counter, increments with each form evaluated.")

(defun default-repl-prompter ()
  "Genera-inspired prompt: PACKAGE(n): where n is command number.
   In debug/nested mode: [level,context] PACKAGE(n):"
  (fresh-line)
  (when (or (plusp *repl-level*) *repl-prompt-context*)
    (format t "[~D~@[,~A~]] " *repl-level* *repl-prompt-context*))
  (format t "~A(~D): " (package-name *package*) (incf *command-counter*)))
