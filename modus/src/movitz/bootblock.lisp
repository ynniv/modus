...
(defun mkasm16-bios-bootloader (image-size load-address &optional (skip-sectors 0) (drive-number 0))
  "Create BIOS bootloader. drive-number: 0=floppy A:, #x80=first HD"
  (let* ((first-sector (1+ skip-sectors))
    ...
    `((:jmp (:pc+ 0))		   ; some BIOSes might check for this.
      ...
      ;; Poll the drive's sectors per track (or use fixed for HD)
      ,@(if (>= drive-number #x80)
            ;; Hard disk: use fixed geometry (63 sectors/track is standard)
            `((:movw 63 (:bp ,+sectors-per-track+)))
            ;; Floppy: probe sectors per track
            `((:movw 5 (:bp ,+sectors-per-track+))
              ...
              (:movw #x0201 :ax)
              (:movw ,drive-number :dx)     ; DL=drive, DH=head 0
      ...
      (:divb :cl :ax)		; al=quotient (head*heads+track), ah=remainder (sector-1)

      (:movb :ah :cl)			; sector - 1 (will be incremented later)
      ,@(if (>= drive-number #x80)
            ;; HD: 16 heads
            `((:xorb :ah :ah)		; clear ah for word division
              (:movb 16 :ch)		; divisor = 16 heads
              (:divb :ch :ax)		; al=cylinder, ah=head
              (:movb :ah :dh)		; head
              (:movb :al :ch))		; cylinder
            ;; Floppy: 2 heads
            `((:movb :al :dh)
              (:andb 1 :dh)		; head = quotient & 1
              (:movb :al :ch)
              (:shrb 1 :ch)))		; cylinder = quotient >> 1
      (:movb ,drive-number :dl)		; drive number (0=floppy, #x80=HD)
...
(defun make-bootblock (image-size load-address call-address
		       &key (skip-sectors 0) (include-records) (drive-number 0))
  "Create bootblock. drive-number: 0=floppy A:, #x80=first hard disk"
  (when (and *floppy-size* (zerop drive-number))
  ...
  (multiple-value-bind (bios-loader bb-symtab)
	...
	(asm:assemble-proglist (mkasm16-bios-bootloader image-size load-address skip-sectors drive-number)
			       :start-pc #x7c00))
