...
(require :lib/net/tls13)
(require :lib/net/ssh)
(require :lib/net/websocket)
(require :lib/json)
(require :lib/net/nostr)
(require :lib/crypto/nip04)
(require :lib/net/nip46)
...
(defpackage modus
  (:nicknames #:los0)  ; Keep los0 as nickname for compatibility
  (:use #:common-lisp
	#:muerte
	#:muerte.lib
	#:muerte.x86-pc
...
(defun modus-debugger (condition)
...
(defun random (limit &optional state)
  ...
  (etypecase limit
    (fixnum
     ;; LCG: x = (a*x + c) mod m
     ;; a=997, c=1, m=65521 (largest prime < 2^16)
     ;; Max intermediate: 997 * 65520 = 65,343,440 fits in 30-bit fixnum
     (when (zerop *random-x*)
       ;; Seed from timestamp
       (setf *random-x* (logand (read-time-stamp-counter) #xFFFF))
       (when (zerop *random-x*)
         (setf *random-x* 12345)))
     ;; LCG step
     (let* ((ax (the fixnum (* 997 *random-x*)))
            (axc (the fixnum (+ ax 1)))
            (newx (rem axc 65521)))
       (setf *random-x* newx)
       (rem newx limit)))
...
(defun genesis ()
  ;; Initialize serial port early for boot messages (COM1 at 115200 baud)
  (let ((uart-base #x3f8))
    (setf (io-port (+ uart-base 3) :unsigned-byte8) #x83)  ; LCR: DLAB=1
    (setf (io-port (+ uart-base 0) :unsigned-byte8) 1)     ; Divisor low (115200)
    (setf (io-port (+ uart-base 1) :unsigned-byte8) 0)     ; Divisor high
    (setf (io-port (+ uart-base 3) :unsigned-byte8) #x03)  ; LCR: 8N1, DLAB=0
    (setf (io-port (+ uart-base 4) :unsigned-byte8) #x03)) ; MCR: DTR+RTS

  (setf *debugger-function* #'modus-debugger)
  (let ((extended-memsize 0))
    ...
    (let* ((buf (check-the fixnum (%run-time-context-slot nil 'muerte::nursery-space)))
      ...
      (cond
        ((< free-kb 1)
         nil)
        (t (install-los0-consing :kb-size (truncate free-kb 2)))))) ;; closes let*, let
  ...
  (setf *package* (find-package "MODUS"))
  ...
  (let ((*repl-readline-context* (make-readline-context :history-size 16))
	...
	#+ignore (*debugger-function* #'modus-debugger)
	#+ignore (*package* *package*))
    (with-simple-restart (abort "Skip Los0 boot-up initialization.")
      (setf *cpu-features*
	(find-cpu-features))
      (muerte.x86-pc.serial:boot-log "CPU features:~:[ none~;~{ ~A~#[~; and~:;,~]~}~]."
	...
	(muerte.x86-pc.serial:boot-log "Warning: No time-stamp-counter. Timer functions unavailable."))
       (t (install-internal-time)
	  (muerte.x86-pc.serial:boot-log "Internal-time will wrap in ~D days."
...
;;;     (format t "~&Installed serial-port write-char."))
    (let ((* nil) (** nil) (*** nil)
      ...
      ;; Auto-enable serial console for REPL I/O
      (ignore-errors
        (muerte.x86-pc.serial:enable-serial-console :baudrate 115200 :echo-p nil))
      ;; Enable serial input right before REPL starts
      (ignore-errors
        (muerte.x86-pc.serial:enable-serial-input))
      (handler-bind
	  ((readline-keypress
	    (lambda (c)
