...
;; Note: The actual random function is defined in los0.lisp with proper LCG implementation
;; This fallback is kept for compatibility with code that might reference it
(defun random (limit &optional (state *random-state*))
  "Return a pseudo-random number between 0 and LIMIT-1.
   This is a fallback - los0.lisp provides the main implementation."
  (declare (ignore state))
  ;; Simple fallback using timestamp
  (rem (logand (read-time-stamp-counter) #xFFFFFF) limit))
