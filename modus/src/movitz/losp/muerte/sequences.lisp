  ...
  (cond
   ...
   ((member result-type '(vector simple-vector))
    (apply 'map-for-vector
	   ...
	   function first-sequence more-sequences))
   ;; Handle typed vector specifications like (vector (unsigned-byte 8))
   ((and (consp result-type)
         (member (car result-type) '(vector simple-vector array simple-array)))
    (let ((element-type (if (cdr result-type) (cadr result-type) t)))
      (apply 'map-for-vector
             (make-array (length first-sequence) :element-type element-type)
             function first-sequence more-sequences)))
   (t (error "MAP result-type ~S not supported." result-type))))
