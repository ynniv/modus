...
(eval-when (:load-toplevel)
  (defun get-setf-expansion (place &optional env)
    "Runtime version of get-setf-expansion with support for common places."
    (declare (ignore env))
    (cond
      ((symbolp place)
       (let ((store-var (gensym "store-var-")))
	 (values nil nil (list store-var) `(setq ,place ,store-var) place)))
      ((and (consp place) (symbolp (car place)))
       ;; Handle common place forms that have (setf foo) functions
       (let* ((accessor (car place))
              (args (cdr place))
              (store-var (gensym "store-var-"))
              (tmp-vars (mapcar (lambda (x) (declare (ignore x)) (gensym "tmp-")) args))
              (tmp-forms args))
         (case accessor
           ;; Places with (setf foo) functions defined
           ((aref svref char schar bit sbit row-major-aref
             gethash symbol-value symbol-function symbol-plist
             fill-pointer)
            (values tmp-vars tmp-forms (list store-var)
                    `(funcall #'(setf ,accessor) ,store-var ,@tmp-vars)
                    `(,accessor ,@tmp-vars)))
           ;; List accessors
           ((car first)
            (values tmp-vars tmp-forms (list store-var)
                    `(progn (rplaca ,(first tmp-vars) ,store-var) ,store-var)
                    `(car ,@tmp-vars)))
           ((cdr rest)
            (values tmp-vars tmp-forms (list store-var)
                    `(progn (rplacd ,(first tmp-vars) ,store-var) ,store-var)
                    `(cdr ,@tmp-vars)))
           ((cadr second)
            (values tmp-vars tmp-forms (list store-var)
                    `(progn (rplaca (cdr ,(first tmp-vars)) ,store-var) ,store-var)
                    `(cadr ,@tmp-vars)))
           ((cddr)
            (values tmp-vars tmp-forms (list store-var)
                    `(progn (rplacd (cdr ,(first tmp-vars)) ,store-var) ,store-var)
                    `(cddr ,@tmp-vars)))
           ((caar)
            (values tmp-vars tmp-forms (list store-var)
                    `(progn (rplaca (car ,(first tmp-vars)) ,store-var) ,store-var)
                    `(caar ,@tmp-vars)))
           ((cdar)
            (values tmp-vars tmp-forms (list store-var)
                    `(progn (rplacd (car ,(first tmp-vars)) ,store-var) ,store-var)
                    `(cdar ,@tmp-vars)))
           ((nth)
            (let ((n-tmp (first tmp-vars))
                  (list-tmp (second tmp-vars)))
              (values tmp-vars tmp-forms (list store-var)
                      `(progn (rplaca (nthcdr ,n-tmp ,list-tmp) ,store-var) ,store-var)
                      `(nth ,@tmp-vars))))
           (t
            ;; Try generic (setf foo) function approach
            (values tmp-vars tmp-forms (list store-var)
                    `(funcall #'(setf ,accessor) ,store-var ,@tmp-vars)
                    `(,accessor ,@tmp-vars))))))
      (t (error "Place ~S not implemented." place)))))
