;;;; TLS 1.3 Implementation — reference CL implementation (needs MVM adaptation)
...
;;;; SHA-384's 64-bit arithmetic requires 63-bit fixnums (MVM).
...
(defun tls-throughput-test (&optional (host "example.com") (port 443))
  ...
  (let ((conn (tls-connect host port)))
    ...
    (let ((request (format nil "GET / HTTP/1.1~c~cHost: ~a~c~cUser-Agent: Modus/1.0~c~cAccept: */*~c~cConnection: close~c~c~c~c"
                           #\return #\newline
                           host
                           #\return #\newline
                           #\return #\newline
