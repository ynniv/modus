;;;; build-aarch64-isolated.lisp - Build AArch64 SSH kernel with actor isolation
;;;;
;;;; Like build-aarch64-actors.lisp but adds Qubes-like isolation:
;;;; net-domain (actor 2) owns all hardware; SSH handlers communicate via mailbox.
;;;;
;;;; Usage: cd lib/modus64 && sbcl --script mvm/build-aarch64-isolated.lisp
;;;;
;;;; Produces /tmp/modus64-aarch64-isolated.bin — boot with:
;;;;   qemu-system-aarch64 -machine virt -cpu cortex-a57 -m 512 \
;;;;     -kernel /tmp/modus64-aarch64-isolated.bin -nographic -semihosting \
...
;;; ============================================================
;;; Load REPL source + networking + actor + isolation source
;;; ============================================================

(format t "Loading REPL + networking + actor + isolation source...~%")
...
;;; ============================================================
;;; Build AArch64 Isolated Actors+SSH image (virt + E1000)
;;; ============================================================
...
(let* ((ssh-main (format nil "~{~A~%~}"
       ...
       (combined-source (concatenate 'string
                                     ...
                                     cl-user::*net-source*
                                     *repl-source*)))
  (format t "Building AArch64 Isolated Actors+SSH image...~%")
  ...
  (let ((image (build-image :target :aarch64 :source-text combined-source)))
    ...
    (write-kernel-image image "/tmp/modus64-aarch64-isolated.bin")
    (format t "Done. Boot with:~%")
    (format t "  qemu-system-aarch64 -machine virt -cpu cortex-a57 -m 512 \\~%")
    (format t "    -kernel /tmp/modus64-aarch64-isolated.bin -nographic -semihosting \\~%")
